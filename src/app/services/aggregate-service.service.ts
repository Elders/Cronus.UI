...
export class AggregateServiceService {
  ...
  getAggregates(): Observable<IResults> {
    return this.http.get<IResults>('http://192.168.10.160:9000/EventStore/explore?id=urn:mv:ticket:01f5a448-a08e-45bc-85ff-a57824835810');
  }

  getAggregatesId(url: string, id: string): Observable<IResults> {
    return this.http.get<IResults>('http://' + url + '/EventStore/explore?id=' + id);
  }

  getToken(url: string, credentials: any, headers: any): Observable<any> {
    return this.http.post<any>('https://' + url, credentials, { headers: headers });
  }
}
